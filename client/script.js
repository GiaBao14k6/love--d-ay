...
function setupAppEvents() {
    ...
    document.getElementById('diary-entries-grid').addEventListener('scroll', function () {
        const container = this;
        if (container.scrollTop + container.clientHeight >= container.scrollHeight - 10) {
            if (currentPage < totalPages) {
                currentPage++;
                fetchDiaryEntries(currentPage, true);
            }
        }
    });
    ...
    comments.forEach(comment => {
        ...
        commentEl.innerHTML = `
            ...
            <div class="comment-footer">
                ...
                <button class="edit-comment-btn" data-entry-id="${entryId}" data-comment-id="${comment._id}" data-author="${comment.author}" data-comment-content="${comment.content}">Sửa</button>
        ...
        comment.replies.forEach(reply => {
            ...
            replyEl.innerHTML = `
                <div class="comment-header">
                    <span class="author-name">${reply.author}</span> (Trả lời)
...
async function handleFormSubmit(event) {
    ...
    try {
        if (entryId) {
            response = await fetch(`${API_BASE_URL}/${entryId}`, {method: 'PUT', body: formData});
        } else {
